...
func upCreateTableVotersView(tx *sql.Tx) error {
	_, err := tx.Exec(`
		...
		select user_address,balance_of(user_address) as bond_staked,
		   ...
		   ( select count(*) from governance_votes where lower(user_id) = lower(supernova_users.user_address) ) +
		   ( select count(*) from governance_abrogation_votes where lower(user_id) = lower(supernova_users.user_address) ) as votes,
		   ( select count(*) from governance_proposals where lower(proposer) = lower(supernova_users.user_address) )         as proposals,
		   voting_power(user_address)                                                                     as voting_power ,
		   has_active_delegation(user_address)
	from supernova_users;
	    `)
	return err
}
