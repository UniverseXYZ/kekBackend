...
var log = logrus.WithField("module", "storable(auction)")
...
func (a *Storable) ToDB(tx *sql.Tx) error {
	...
	for _, data := range a.raw.Receipts {
		for _, log := range data.Logs {
			...
			if utils.LogIsEvent(log, a.auctionAbi, Deposit) {
				d, err := a.decodeLog(log, Deposit)
				if err != nil {
					return err
				}
			...
			if utils.LogIsEvent(log, a.auctionAbi, Withdraw) {
				w, err := a.decodeLog(log, Withdraw)
				if err != nil {
					return err
				}
	...
	err := a.storeActions(tx, stakingActions)
	if err != nil {
		return err
	}
...
func (a Storable) decodeLog(log web3types.Log, event string) (*StakingAction, error) {
	var d StakingAction
	...
	err = a.auctionAbi.UnpackIntoMap(extraData, event, data)
	if err != nil {
...
func (a Storable) storeActions(tx *sql.Tx, actions []StakingAction) error {
